...
static const char *WINAMP_HTML =
    ...
    "    <button id=\"mi-play\" class=\"ctx-item\" onclick=\"onMenuPlayPause()\">Play / Pause</button>"
    "    <button id=\"mi-stop\" class=\"ctx-item\" onclick=\"onMenuStop()\">Stop</button>"
    "    <button id=\"mi-prev\" class=\"ctx-item\" onclick=\"onMenuPrev()\">Previous track</button>"
    "    <button id=\"mi-next\" class=\"ctx-item\" onclick=\"onMenuNext()\">Next track</button>"
    "    <div class=\"ctx-sep\"></div>"
    "    <button id=\"mi-rew\" class=\"ctx-item\" onclick=\"onMenuRew()\">Rewind 10s</button>"
    "    <button id=\"mi-fwd\" class=\"ctx-item\" onclick=\"onMenuFwd()\">Forward 10s</button>"
    "    <button id=\"mi-volup\" class=\"ctx-item\" onclick=\"onMenuVolUp()\">Volume up</button>"
    "    <button id=\"mi-voldn\" class=\"ctx-item\" onclick=\"onMenuVolDown()\">Volume down</button>"
    "    <div class=\"ctx-sep\"></div>"
    "    <button id=\"mi-eq\" class=\"ctx-item\" onclick=\"onMenuToggleEq()\">Equalizer</button>"
    "    <button id=\"mi-pl\" class=\"ctx-item\" onclick=\"onMenuTogglePlaylist()\">Playlist editor</button>"
    "    <button id=\"mi-add-folder\" class=\"ctx-item\" onclick=\"onAddFolder()\">Add folder...</button>"
    "    <div class=\"ctx-sep\"></div>"
    "    <button id=\"mi-xtc\" class=\"ctx-item\" onclick=\"onMenuXtc()\">Crosstalk (XTC)</button>"
    "    <button id=\"mi-xtc-up\" class=\"ctx-item\" onclick=\"onMenuXtcUp()\">XTC atten +</button>"
    "    <button id=\"mi-xtc-dn\" class=\"ctx-item\" onclick=\"onMenuXtcDown()\">XTC atten -</button>"
    "    <button id=\"mi-dsd\" class=\"ctx-item\" onclick=\"onMenuDsd()\">DSD (DoP)</button>"
    "    <button id=\"mi-sr\" class=\"ctx-item\" onclick=\"onMenuSr()\">Super resolution</button>"
    "    <button id=\"mi-loop\" class=\"ctx-item\" onclick=\"onMenuLoop()\">Repeat playlist</button>"
    "    <button id=\"mi-fmt\" class=\"ctx-item\" onclick=\"onMenuFmt()\">Format filter</button>"
    "    <button id=\"mi-dev\" class=\"ctx-item ctx-has-sub\" onclick=\"onMenuDevice()\">ALSA device</button>"
    "    <div class=\"ctx-sep\"></div>"
    "    <button id=\"mi-skin-rand\" class=\"ctx-item\" onclick=\"onSkinRandom()\">Random skin / track</button>"
    "    <button id=\"mi-skin-next\" class=\"ctx-item\" onclick=\"onSkinNext()\">Next skin</button>"
    "    <button id=\"mi-skin-folder\" class=\"ctx-item\" onclick=\"onSkinFolder()\">Skin folder...</button>"
    "    <button id=\"mi-skin-builtin\" class=\"ctx-item\" onclick=\"onSkinBuiltin()\">Built-in Ember skin</button>"
    "    <div class=\"ctx-sep\"></div>"
    "    <button id=\"mi-about\" class=\"ctx-item\" onclick=\"onAbout()\">About aplay+</button>"
    "    <button id=\"mi-exit\" class=\"ctx-item ctx-danger\" onclick=\"onMenuQuit()\">Exit</button>"
    "  </div>"
    ...
    "    <button class=\"about-close\" aria-label=\"Close\" onclick=\"onAboutClose()\"></button>"
...
static const char *WINAMP_CSS =
    ...
    ".skin-list-row{width:239px;height:12px;padding:0 2px;font-family:var(--skin-font);font-size:10px;line-height:12px;color:var(--playlist-normal);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer;}"
    ...
    ".skin-device,.skin-note{position:absolute;top:217px;height:10px;font-family:var(--skin-font);font-size:6px;line-height:9px;color:var(--playlist-normal);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
    ".skin-device{left:14px;width:100px;cursor:pointer}.skin-note{left:120px;width:141px;text-align:right;}"
    ".ctx-menu{position:absolute;left:0;top:250px;width:260px;height:560px;padding:6px 0;background:#1a1612;border:1px solid #5c3a22;box-shadow:2px 2px 0 rgba(0,0,0,.45);z-index:50;}"
    /* Menu is a native-sized popup (not the Winamp 1x skin grid). Larger
     * glyphs avoid the blurry look you get when 10px bold mono is stretched. */
    ".ctx-item{display:block;width:100%;height:18px;padding:0 12px;border:0;background:transparent;color:#E4D6C5;font:12px/18px 'DejaVu Sans',sans-serif;text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
    ".ctx-item:hover{background:#5C3A22;color:#FFE6B8;}"
    ".ctx-item.checked{color:#FFE6B8;}"
    ".ctx-item.ctx-slot{display:none;}"
    ".ctx-item.ctx-slot.ctx-show{display:block;}"
    ".ctx-has-sub:after{content:'\\25B8';float:right;color:#9a7f66;}"
    ".ctx-danger{color:#e8a0a0;}"
    ".ctx-sep{height:1px;margin:4px 10px;background:#5c3a22;opacity:.85;}"
    ".ctx-submenu{z-index:55;}"
    ".ctx-submenu-cards{left:270px;top:250px;width:240px;height:420px;}"
    ".ctx-submenu-pcms{left:520px;top:250px;width:240px;height:420px;}"
    ".ctx-heading{height:22px;padding:0 12px;color:#9a7f66;font:700 10px/22px 'DejaVu Sans',sans-serif;letter-spacing:0.06em;text-transform:uppercase;}"
    ".about-dialog{position:absolute;left:520px;top:680px;width:340px;height:400px;padding:0;background:linear-gradient(165deg,#241e19 0%,#1a1612 48%,#12100e 100%);border:1px solid #7a4a28;box-shadow:0 18px 40px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,214,170,.12);z-index:60;}"
    ".about-close{position:absolute;right:10px;top:10px;width:14px;height:14px;padding:0;border:1px solid #5c3a22;background:#2a221c;color:#E4D6C5;cursor:pointer;}"
    ".about-close:after{content:'\\00d7';display:block;font:700 12px/12px 'DejaVu Sans',sans-serif;text-align:center;color:#E4D6C5;}"
    ".about-mark{position:absolute;left:24px;top:28px;width:52px;height:52px;border-radius:14px;background:linear-gradient(145deg,#e8a868,#a85a28);box-shadow:0 10px 24px rgba(168,90,40,.35),inset 0 1px 0 rgba(255,255,255,.25);color:#1a120c;font:700 28px/52px 'DejaVu Serif',serif;text-align:center;}"
    ".about-brand{position:absolute;left:92px;top:34px;color:#FFE6B8;font:700 28px/1 'DejaVu Serif',serif;letter-spacing:-0.02em;}"
    ".about-tag{position:absolute;left:92px;top:68px;display:flex;align-items:center;gap:6px;padding:3px 10px;border:1px solid rgba(232,168,104,.35);border-radius:999px;background:rgba(232,168,104,.10);color:#E4D6C5;font:600 10px/1.2 'DejaVu Sans',sans-serif;letter-spacing:0.04em;}"
    ".about-dot{width:5px;height:5px;border-radius:50%;background:#e8a868;}"
    ".about-sep{position:absolute;left:24px;top:104px;width:292px;height:1px;background:linear-gradient(90deg,transparent,#5c3a22,transparent);}"
    ".about-body{position:absolute;left:24px;top:122px;width:292px;color:#cbb7a2;font:13px/1.55 'DejaVu Sans',sans-serif;}"
    ".about-specs{position:absolute;left:24px;top:220px;width:292px;}"
    ".about-spec{display:flex;justify-content:space-between;align-items:center;height:28px;margin:0 0 6px;padding:0 10px;border:1px solid #3a2a20;border-radius:8px;background:rgba(255,255,255,.03);}"
    ...
    ".about-ok{position:absolute;left:24px;bottom:22px;width:292px;height:34px;border:1px solid #7a4a28;border-radius:8px;background:linear-gradient(180deg,#5c3a22,#3a2416);color:#FFE6B8;font:700 12px/34px 'DejaVu Sans',sans-serif;cursor:pointer;}"
...
static void aui_set_menu_check(int idx, int on, const char *label)
{
	...
	if (on) luna_add_class(idx, "checked");
	else luna_remove_class(idx, "checked");
}
...
static void gui_cache_elements(void)
{
	...
	g_el_mi_dev = luna_get_element_by_id("mi-dev");
	for (int i = 0; i < AUI_MAX_DEV_CARDS; i++) {
		char id[8];
	...
	g_el_mi_volup      = luna_get_element_by_id("mi-volup");
	g_el_mi_voldn      = luna_get_element_by_id("mi-voldn");
	g_el_mi_xtc        = luna_get_element_by_id("mi-xtc");
	g_el_mi_xtc_up     = luna_get_element_by_id("mi-xtc-up");
	g_el_mi_xtc_dn     = luna_get_element_by_id("mi-xtc-dn");
	g_el_mi_dsd        = luna_get_element_by_id("mi-dsd");
	g_el_mi_sr         = luna_get_element_by_id("mi-sr");
	g_el_mi_loop       = luna_get_element_by_id("mi-loop");
	g_el_mi_skin_rand  = luna_get_element_by_id("mi-skin-rand");
	g_el_mi_eq         = luna_get_element_by_id("mi-eq");
	g_el_mi_pl         = luna_get_element_by_id("mi-pl");
	for (int i = 0; i < 13; i++) {
		char id[8];
...
static void gui_run(void)
{
	...
	g_surfaces[AUI_MENU] = (AuiSurface){ .kind=AUI_MENU, .width=260, .height=560, .origin_x=0, .origin_y=250, .visible=0 };
	g_surfaces[AUI_DEV_CARDS] = (AuiSurface){ .kind=AUI_DEV_CARDS, .width=240, .height=420, .origin_x=270, .origin_y=250, .visible=0 };
	g_surfaces[AUI_DEV_PCMS] = (AuiSurface){ .kind=AUI_DEV_PCMS, .width=240, .height=420, .origin_x=520, .origin_y=250, .visible=0 };
	g_surfaces[AUI_ABOUT] = (AuiSurface){ .kind=AUI_ABOUT, .width=340, .height=400, .origin_x=520, .origin_y=680, .visible=0 };
...
int main(int argc, char *argv[])
{
	...
	gui_bridge_init();
	parg_init(&ps);
	while ((c = parg_getopt(&ps, argc, argv, "hS:R:d:frxs:t:pclvDTVeo:")) != -1) {
		switch (c) {
